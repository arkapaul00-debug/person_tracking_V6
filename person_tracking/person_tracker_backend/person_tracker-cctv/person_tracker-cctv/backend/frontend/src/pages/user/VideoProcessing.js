...
const VideoProcessing = {
    ...
    async startProcessing() {
        ...
        this._progress = 0;
        SentinelToast.info('Uploading...', 'Starting video processing on the backend.');

        const formData = new FormData();
        formData.append('video', this._uploadedVideo);
        ...
        try {
            const result = await SentinelAPI.analyzeVideo(formData);
            ...
            // Navigate to Post Processing Live and pass the case ID
            location.hash = `#/user/post-processing-live?case=${result.case_id}`;
        } catch (error) {
            SentinelToast.error('Upload Failed', error.message);
