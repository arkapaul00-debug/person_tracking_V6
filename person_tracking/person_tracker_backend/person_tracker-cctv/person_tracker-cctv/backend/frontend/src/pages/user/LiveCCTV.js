...
const LiveCCTV = {
        ...
        return `
            <div class="stream-preview-card" id="preview-card-${stream.id}">
                <div class="stream-preview-thumb">
                    <img src="/api/live/snapshot/${stream.id}/" class="fs-feed-canvas" style="width:100%;height:100%;object-fit:cover;position:absolute;inset:0;z-index:1;" onerror="setTimeout(()=>this.src=this.src.split('?')[0]+'?t='+Date.now(), 2000)">
                    <div class="stream-preview-overlay" style="z-index:2;">
                        <div>
            ...
            <div class="fs-overlay" id="fs-overlay">
                ...
                <div class="fs-main-area">
                    ...
                    <div class="fs-main-feed ${this._detectedStreamIds.has(mainStream.id) ? 'suspect-detected' : ''}" id="fs-main-feed">
                        <img src="/api/live/mjpeg/${mainStream.id}/" class="fs-feed-canvas" style="width:100%;height:100%;object-fit:cover;" onerror="setTimeout(()=>this.src=this.src.split('?')[0]+'?t='+Date.now(), 2000)">
                    ...
                    <div class="fs-stream-grid" id="fs-stream-grid">
                        ${renderedOthers.map(s => `
                            <div class="fs-stream-cell ${this._detectedStreamIds.has(s.id) ? 'detected' : ''}"
                                ...
                                <img src="/api/live/mjpeg/${s.id}/" class="fs-feed-canvas" style="width:100%;height:100%;object-fit:cover;" onerror="setTimeout(()=>this.src=this.src.split('?')[0]+'?t='+Date.now(), 2000)">
    ...
    async addStream(e) {
        ...
        try {
            const result = await SentinelAPI.addStream(data);
            data.id = result.id;
            ...
            SentinelToast.error('Failed to add stream', error.message);
        }
    },
    ...
    async startSession() {
        ...
        formData.append('mode', this._detectionMode);
        formData.append('threshold', (this._threshold / 100).toFixed(2));
        ...
        this._targetImages.forEach(img => {
            formData.append('references', img.file || img.data); 
        });
        
        SentinelToast.info('Starting Live Session...', 'Initializing backend tracking pipelines.');

        try {
            const result = await SentinelAPI.startLiveStream(formData);
            this._sessionId = result.case_id || result.session_id;
            this._isLive = true;
            ...
            SentinelToast.error('Failed to start session', error.message);
        }
    },
    ...
    stopSession() {
        SentinelModal.confirm({
            ...
            onConfirm: async () => {
                try {
                    await SentinelAPI.stopLiveStream();
                    ...
                    SentinelToast.error('Failed to stop session', error.message);
                }
            }
        });
    },
    ...
    _startLivePolling() {
        ...
        this._alertInterval = setInterval(async () => {
            if (!this._isLive || !this._fullscreenActive || !this._sessionId) return;
            
            try {
                // Fetch alerts from backend
                const data = await SentinelAPI.getLiveAlerts(this._sessionId);
                ...
                if (newAlerts.length > 0) {
                    for (const alert of newAlerts.reverse()) {
                        ...
                        // Add to detected set for UI highlight
                        this._detectedStreamIds.add(alert.stream_id);
                        this._focusedStreamId = alert.stream_id;
                        
                        // Store alert in local UI state
                        ...
                        SentinelStore.addEvidence({
                            type: 'live',
                            cameraName: alert.stream,
                            cameraLocation: 'Live Stream',
                            detectionMode: this._detectionMode,
                            confidence: confidence + '%',
                            duration: '15s',
                            title: `Detection - ${alert.stream}`,
                            targetName: this._targetName || 'Unknown Suspect',
                            clipUrl: alert.clip_url,
                            createdAt: alert.timestamp
                        });
                        
                        SentinelStore.addActivity({
                            type: 'alert',
                            message: `Suspect detected on ${alert.stream} (${confidence}%)`,
                            color: 'red'
                        });
                        
                        // Clear detection highlight after 8 seconds
                        setTimeout(() => {
                            this._detectedStreamIds.delete(alert.stream_id);
                            if (this._fullscreenActive) {
                                const sidebarEl = document.getElementById(`fs-sidebar-${alert.stream_id}`);
                                if (sidebarEl) sidebarEl.classList.remove('detected');
                                const cellEl = document.getElementById(`fs-cell-${alert.stream_id}`);
                ...
                console.error('Error polling live alerts:', error);
            }
